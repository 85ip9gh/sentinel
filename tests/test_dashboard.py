...
def test_the_newest_reading_per_host_wins():
    ...
    _write(
        ...
        "2026-08-15",
        [
            _system("g7-server", "2026-08-15T17:04:00Z", cpu=1.0),
            _system("g7-server", "2026-08-15T17:04:50Z", cpu=9.0),
            _system("cubebox", "2026-08-15T17:04:30Z", cpu=4.0),
        ],
    )

    status = build_status(_archive(writer), NOW)

    assert [h["host"] for h in status["hosts"]] == ["cubebox", "g7-server"]
    assert status["hosts"][1]["cpu_percent"] == 9.0


def test_a_host_card_reports_its_own_age_and_freshness():
    writer = FakeWriter()
    _write(writer, "system", "2026-08-15", [_system("g7-server", "2026-08-15T17:04:30Z")])
...
def test_a_reading_that_stopped_arriving_reads_as_stale():
    writer = FakeWriter()
    _write(writer, "system", "2026-08-15", [_system("g7-server", "2026-08-15T16:00:00Z")])
...
def test_yesterday_is_used_when_today_has_no_partition_yet():
    ...
    writer = FakeWriter()
    _write(writer, "system", "2026-08-14", [_system("g7-server", "2026-08-14T23:59:00Z")])

    status = build_status(_archive(writer), NOW)

    assert [h["host"] for h in status["hosts"]] == ["g7-server"]
...
def test_site_checks_keep_the_newest_row_per_url():
    ...
    assert len(checks) == 2
    assert {c["host"] for c in checks} == {"cubebox", "g7-server"}
...
def test_containers_are_counted_from_the_newest_record_per_container():
    ...
    assert containers == [{"host": "cubebox", "running": 1, "total": 2}]


def test_the_archive_summary_counts_days_and_files():
    writer = FakeWriter()
    _write(writer, "system", "2026-08-14", [_system("g7-server", "2026-08-14T12:00:00Z")])
    _write(writer, "system", "2026-08-15", [_system("g7-server", "2026-08-15T12:00:00Z")], "part-a.ndjson")
    _write(writer, "system", "2026-08-15", [_system("g7-server", "2026-08-15T13:00:00Z")], "part-b.ndjson")
...
def test_the_page_and_the_api_both_serve():
    ...
    assert b"g7-server" in page.data

    api = client.get("/api/status")
    assert api.status_code == 200
    assert api.get_json()["hosts"][0]["host"] == "g7-server"
...
def test_a_reading_stamped_in_the_future_is_not_shown():
    ...
    _write(
        ...
        "2026-08-15",
        [
            _system("g7-server", "2026-08-15T17:04:30Z", cpu=3.0),
            _system("g7-server", "2026-08-15T23:00:00Z", cpu=99.0),
        ],
    )
